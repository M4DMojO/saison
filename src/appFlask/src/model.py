...
from custom_function import get_all_weights_from_bucket, get_result_from_yolo_total
...
def load_models() -> list:
    ...
    """
    base_path = "../models/"
    files = [f for f in os.listdir(base_path) if os.path.isfile(os.path.join(base_path, f)) and ("pt" in f or "keras" in f)]
...
def get_results(model:Model|YOLO|YOLOToVGG, 
    ...
    """
    if index == 0:#yolo
        return get_result_from_yolo_total(model(img))
